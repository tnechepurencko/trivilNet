...
var error_tests = []one{
	...
	{"модуль x; вход { пусть а = ложь{} }", "СЕМ-КОНСТРУКТОР-НЕТ-ТИПА"},
	{"модуль x; вход { пусть а = Лог{} }", "СЕМ-КОН-КЛАССА-ОШ-ТИП"},
	{"модуль x; тип К = класс { ц: Цел64=1}; вход { пусть к = К{я: 1}}", "СЕМ-КОН-КЛАССА-НЕТ-ПОЛЯ"},
	{"модуль x; тип К = класс { ц: Цел64=1}; фн (к: К) я() {}; вход { пусть к = К{я: 1}}", "СЕМ-КОН-КЛАССА-НЕ-ПОЛE"},
	...
	{"модуль x; тип А = []Цел64; вход { пусть а = А[].заполнить() }", "СЕМ-СТДФУНК-ОШ-ЧИСЛО-АРГ"},
	{"модуль x; тип А = []Цел64; вход { пусть а = А[].заполнить(1) }", "СЕМ-СТДФУНК-ОШ-ЧИСЛО-АРГ"},
	{"модуль x; тип А = []Цел64; вход { пусть а = А[].заполнить(1, 2, 3) }", "СЕМ-СТДФУНК-ОШ-ЧИСЛО-АРГ"},

	{"модуль x; тип А = []Цел64; вход { пусть а = А[].заполнить(ложь, 2) }", "СЕМ-НЕСОВМЕСТИМО-ПРИСВ"},
	{"модуль x; тип А = []Цел64; вход { пусть а = А[].заполнить(2, ложь) }", "СЕМ-НЕСОВМЕСТИМО-ПРИСВ"},
