...
var error_tests = []one{
	...
	{"модуль x; вход { пусть а = тег() }", "СЕМ-ОШ-ЧИСЛО-АРГ-СТДФУНК"},
	{"модуль x; вход { пусть а = тег(Лог, Лог) }", "СЕМ-ОШ-ЧИСЛО-АРГ-СТДФУНК"},
	{"модуль x; фн Ф(а: ...Цел64){ пусть б = тег(а) }", "СЕМ-ОЖИДАЛСЯ-ТИП-ИЛИ-ТЕГ-ПАРА"},
